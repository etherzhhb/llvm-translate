//===--- llvm-backport.cpp - The backport LLVM bc to LLVM 3.1 --------------==//
//
...
//   llvm-as [options]      - Read LLVM asm from stdin, write bitcode (v3.1)
//                            to stdout
//   llvm-as [options] x.ll - Read LLVM asm from the x.ll file, write bitcode
//                            (v3.1) to the x.bc file.
//
//===----------------------------------------------------------------------===//
...
static void WriteOutputFile(const Module *M) {
  ...
  if (OutputFilename.empty()) {
    if (InputFilename == "-") {
      ...
      if (IFN.endswith(".ll")) {
        OutputFilename = IFN.drop_back(3).str();
        OutputFilename += ".bc";
      } else
